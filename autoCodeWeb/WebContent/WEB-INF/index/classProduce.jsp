...
<html>
	...
	$(function() {
		...
		$("#templateId").on("change", function(e) {
			if($("#templateId").val()!=""){
				changeProjectSelect();
			}
		});
		...
		$("#openFile").on("click", function(e) {
			$.ajax({
				url : 'produce/openFile.html?fileName=classProduce',
				cache : false,
				dataType : "json",
				success : function(data) {
					if(!data.result){
	...
	function changeTemplateSelect(){
		$("#templateConfigId").html("");
		if($('#templateId').val()==""){
			return;
		}
		$.ajax({
			url : 'templateConfig/templateConfigSelectByTemplateId.html?templateId='+$('#templateId').val(),
			data:{
				"a":Math.random()
			},
			cache : false,
			...
			success : function(data) {
				if(data.result){
					for ( var i = 0; i < data.dataList.length; i++) {
						$("#templateConfigId").append(data.dataList[i]);
					}
					$("#templateConfigId").select2();
				}else{
					showMessage(data.result,decodeURI(data.message));
				}
			}
	...
	function classProduce() {
		...
		templateConfigId=$("#templateConfigId").val();
		if(projectId==null){
		...
		if(templateConfigId==null){
			showMessage(false,  "请先选择模板类型后在生成...");
			return;
		}
		bootbox.confirm("是否生成"+$("#projectId").find("option:selected").text()+"项目？", function(result) {
			if (result) {
				$.ajax({
					url : 'produce/startClassProduce.html?projectId='+projectId+"&templateId="+templateId+"&templateConfigId="+templateConfigId,
					cache : false,
					dataType : "json",
					success : function(data) {
						if(data.result){
	...
	function readProjectProduceState(){
		...
		$.ajax({
				url : 'produce/readClassProduceState.html?projectName='+projectName,
				cache : false,
				dataType : "json",
				success : function(data) {
					if(data.result){
					...
					<div class="row">
						<div class="col-xs-12">
							<div class="col-xs-5">
								...
								<select id="templateConfigId" name="templateConfigId" style="width: 160px;">
								</select>
							</div>
							<div class="col-xs-7">
								<button type="button" class="btn btn-primary" onclick="classProduce();">
