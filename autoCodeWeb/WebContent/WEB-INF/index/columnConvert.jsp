...
<html>
	...
	<script>
		var columnsZh="序号,字段名称,转换名称,中文名称";
		var columnsEn="columnConvertId,mappingName,columnName,columnZhName";	//对应的对象属性
		var columnsDb="columnConvertId,mappingName,columnName,columnZhName";	//对应的数据库字段
		var isConvert=false;
		...
		$(function() { 
			requestUrl={
				'append':'columnConvert/insertColumnConvert.html',
				'update':'columnConvert/updateColumnConvert.html',
				'deleteUrl':'columnConvert/deleteColumnConvert.html',
				'deletes':'columnConvert/deleteColumnConverts.html',
				'query':'columnConvert/queryColumnConvertList.html'
			}
		});

		function importFile(){
			$("#excelModal").modal();
			var chuangkou=window.open('columnConvert/uploadFile.html?toPage=false','Excel导入数据','height=300px,width=480px,top=120px;,left=450px,toolbar=no,menubar=no,scrollbars=no,resizable=no,location=no,status=no') 
		}

		function oneKeyUpdate(){
			$.ajax({
				url : 'columnConvert/oneKeyUpdate.html',
				data:{
					"a":Math.random()
				},
				cache : false,
			...
			<div class="panel-body">
				<div class="row">
					<div class="col-xs-4">
						...
						<button class="btn btn-primary" onclick="oneKeyUpdate()"><i class="glyphicon glyphicon-ok"></i>更正 </button>
						<button class="btn btn-danger" id="removes"><i class="glyphicon glyphicon-trash"></i>删 除 </button>
						<button class="btn btn-success" onclick="importFile()"><i class="glyphicon glyphicon glyphicon-upload"></i>导入文件</button>
					</div>
				  	<div class="col-xs-7">
						...
						<div class="row">
							<div class="col-xs-12">
								<form id="objectForm" method="post" onsubmit="return false;">
									<input type="hidden" name="columnConvertId" id="columnConvertId">
